...
spendTopCountryPlot <- ggplot(topCountrySpend, aes(x=market, y=sum, fill=market))+geom_bar(stat="identity",position="dodge")+labs(x="Pays",y="D�penses (milliers de livres)",fill = "Pays")
...
ggplot(spendPerMarket, aes(x=year, y=sum, colour=market))+geom_line() + geom_point()+labs(title="D�penses en fonction du pays d'origine et de l'ann�e",x="Ann�es",y="D�penses (milliers de livres)",color = "Pays d'origine")

#Depenses en fonction de l'annee et du motif
spendsPerPurposeAndYear <-ddply(visitsp5, .(year, purpose), summarise, sum=sum(spend))
ggplot(spendsPerPurposeAndYear, aes(x=year, y=sum, colour=purpose))+geom_line() + geom_point()+labs(title="D�penses en fonction du motif et de l'ann�e",x="Ann�es",y="D�penses (milliers de livres)",color = "Motif")
